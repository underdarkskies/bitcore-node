...
Ravencoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['listAssetBalancesByAddress', this, this.listAssets, 1],
    ['listAddressesByAsset', this, this.listAssets, 1],
    ['getAssetData', this, this.listAssets, 1],
...
Ravencoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = _.isUndefined(options.assetName) ? addresses.join('') : addresses.concat(assetName).join('');
  ...
  function updateWithMempoolAssets(confirmedUtxos, mempoolDeltas) {
    /* jshint maxstatements: 20 */
    if (!mempoolDeltas || !mempoolDeltas.length) {
      return confirmedUtxos;
    }
  ...
  function finish(mempoolDeltas) {
    if (utxos) {
      ...
      var cbfox = function (err, response) {
        if (err) {
          return callback(self._wrapRPCError(err));
        }
        var utxos = response.result.reverse();
        self.utxosCache.set(cacheKey, utxos);
        callback(null, updateWithMempoolAssets(utxos, mempoolDeltas));
      };
      if (assetName === undefined) {
        self.client.getAddressUtxos({ addresses: addresses }, cb);
      } else {
        self.client.getAddressUtxos({ addresses: addresses, chaininfo: false, assetName: assetName}, cbfox);
      }
    } 
  }
