...
/**
 * Will remove a service from ravencore-node.json
...
/**
 * Will remove the Node.js service and from the ravencore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The ravencore-node configuration directory
...
function remove(options, done) {
  ...
  var ravencoreConfigPath = path.resolve(configPath, 'ravencore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(ravencoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a ravencore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to ravencore-node.json
        removeConfig(ravencoreConfigPath, service, next);
      });
    }, done
  );
}
